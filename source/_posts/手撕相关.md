...
public class DuelSimulation {
    public static void main(String[] args) {
        ...
        int[] a = new int[5]; // 小易的式神的攻击力
        int[] b = new int[5]; // 小易的式神的血量
        int[] c = new int[5]; // 对手的式神的攻击力
        int[] d = new int[5]; // 对手的式神的血量
        ...
        // 记录当前存活的式神编号
        int myCurrent = 0, opponentCurrent = 0;

        while (myCurrent < 5 && opponentCurrent < 5) {
            // 小易攻击对手
            d[opponentCurrent] -= a[myCurrent];
            if (d[opponentCurrent] <= 0) {
                opponentCurrent++;
                if (opponentCurrent >= 5) {
                    break;
                }
            }

            // 对手攻击小易
            b[myCurrent] -= c[opponentCurrent];
            if (b[myCurrent] <= 0) {
                myCurrent++;
            }
        }
        ...
        if (opponentCurrent >= 5) {
            ...
            System.out.println(5 - myCurrent);
        } else {
            System.out.println("lose");
            System.out.println(5 - opponentCurrent);
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int a = A; a >= ai; a--) {
                for (int b = B; b >= bi; b--) {
                    for (int c = C; c >= ci; c--) {
                        // dp[a][b][c]表示使用了a单位的石灰，b单位的砂岩和c单位的花岗岩时可以获得的最大收益。
...
public class ChebyshevDistance {
    private static final int OFFSET = 500; // 偏移量，使坐标变为正数
    private static final int SIZE = 1001;  // 坐标范围[-500, 500]变为[0, 1000]
