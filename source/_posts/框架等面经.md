...
3. 加载自动配置候选类
   - Spring Boot 2.7 引入并推荐从 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` 中读取自动配置类，这个文件中列出了所有自动配置类的全限定类名，SpringBoot通过`SpringFactoriesLoader`读取这些类名，并加载相应的自动配置类。。同时兼容旧的 `META-INF/spring.factories`。
   - Spring Boot 3 已经移除通过 `spring.factories` 注册自动配置的方式，必须使用 `AutoConfiguration.imports`。因此，回答这一步时要注意版本差异。
4. 根据条件过滤配置
...
Starter 的作用是让业务项目引入一个依赖，就能获得相关依赖和默认配置。自定义 Starter 的核心可以分成两部分：Starter 模块负责聚合依赖，AutoConfiguration 模块负责根据条件自动创建 Bean。下面以一个问候服务 `greeting-spring-boot-starter` 为例。
...
4. 注册自动配置类
   - 在 `autoconfigure` 模块中创建文件：

   ```text
   META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports
   ```

   - 在文件中填写自动配置类的全限定名，每行一个：

   ```text
   com.example.greeting.GreetingAutoConfiguration
   ```

   - Spring Boot 2.7 可以同时保留 `AutoConfiguration.imports` 和 `spring.factories`；Spring Boot 3 必须使用 `AutoConfiguration.imports`。
   - 如果需要兼容 Spring Boot 2.6 及更早版本，还要在 `spring.factories` 中声明自动配置类，并把 `@AutoConfiguration` 换成 `@Configuration(proxyBeanMethods = false)`。
5. 聚合 Starter 依赖
...
6. 引入并验证 Starter
   - 业务项目引入 `greeting-spring-boot-starter` 后，只需要添加配置，就可以直接注入 `GreetingService`。
