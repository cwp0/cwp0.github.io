...
这四个注解本质上都属于组件注解，`@Controller`、`@Service` 和 `@Repository` 都基于 `@Component` 定义。被它们标注的类都可以被组件扫描发现，并注册为 Spring Bean，主要区别在于使用场景和附加能力：

- `@Component`：通用组件注解。当一个类不明确属于控制层、业务层或持久层时，可以使用它。
- `@Controller`：用于控制层。Spring MVC 会识别其中的请求处理方法；如果接口需要直接返回 JSON，通常使用由 `@Controller` 和 `@ResponseBody` 组合而成的 `@RestController`。
- `@Service`：用于业务逻辑层，主要表达类的职责，目前没有额外的特殊功能，但有助于提高代码可读性，也方便针对业务层统一配置 AOP。
- `@Repository`：用于持久层。除了表达分层语义，还可以配合 Spring 的异常转换机制，将不同持久化框架的异常转换为统一的 `DataAccessException` 体系。

一句话概括，这四个注解都能把类注册为 Bean，`@Component` 最通用，另外三个分别表示控制层、业务层和持久层，其中 `@Repository` 还可以参与持久化异常转换。
...
Spring 中常用的 Bean 注入注解有 `@Autowired`、`@Resource` 和 `@Inject`，三者都能完成依赖注入，但来源和匹配规则不同。

- `@Autowired`：Spring 提供的注解，默认按类型查找 Bean，可以标注在构造方法、字段、Setter 方法和普通方法上。如果同一类型存在多个 Bean，可以配合 `@Qualifier` 缩小候选范围，或者通过 `@Primary` 指定首选 Bean。从 Spring 4.3 开始，类中只有一个构造方法时可以省略 `@Autowired`。
- `@Resource`：JSR-250 提供的注解，主要用于字段和 Setter 方法。未指定 `name` 时，Spring 会先按字段名或属性名匹配 Bean，匹配不到再按类型查找；显式指定 `name` 后，则按照该名称查找。
- `@Inject`：JSR-330 提供的注解，在 Spring 中的用法与 `@Autowired` 基本一致，也可以配合 `@Named` 或 `@Qualifier` 筛选候选 Bean，但它没有 `required=false` 这一属性。

`@Qualifier` 和 `@Primary` 本身不负责触发依赖注入，而是在存在多个同类型 Bean 时帮助 Spring 选择候选对象。`@Qualifier` 表示进一步限定，`@Primary` 表示没有其他限定条件时优先选择。

实际开发更推荐构造器注入，因为依赖关系明确，也更方便测试。例如：
...
另外，`@Value` 主要用于注入配置属性或 Spring EL 表达式的计算结果，不属于典型的 Bean 注入注解，不应与上面三者混为一谈。

一句话概括，`@Autowired`、`@Resource` 和 `@Inject` 负责注入依赖，`@Qualifier`、`@Primary` 负责解决候选 Bean 的选择问题，`@Value` 则主要负责注入配置值。
...
### 对Spring AOP的理解
面向切面编程(Aspect-Oriented Programming, AOP)是面向对象编程(OOP)的补充。它把日志、事务、权限、监控等分散在各个业务方法中的横切逻辑抽成切面，在不修改核心业务代码的前提下统一增强方法，从而减少重复代码和模块耦合。

Spring AOP 的核心是运行时代理：容器保存的通常不是原始对象，而是代理对象。外部调用先进入代理，由代理匹配切点，再以嵌套方式推进拦截器链；链尾调用目标方法，返回时再执行对应的返回、异常或后置逻辑。Spring 也支持 AspectJ 注解风格，但使用 `@Aspect` 并不代表采用了 AspectJ 字节码织入，Spring AOP 默认仍然是代理机制。

### Spring AOP两种实现方式

**口述版**

Spring AOP 有 JDK 动态代理和 CGLIB 代理两种实现。
- JDK 动态代理基于接口，运行时通过 `Proxy` 和 `InvocationHandler` 生成接口的代理对象；
- CGLIB 基于继承，运行时生成目标类的子类并重写可代理的方法，在方法调用前后执行增强。两者本质都是让调用先经过代理，再进入目标方法。

JDK 代理要求从接口类型调用，CGLIB 不要求目标类实现接口，但不能代理 `final` 类，也不能重写 `final`、`private`、`static` 方法。需要注意，Spring Boot 2.x 以后默认倾向使用 CGLIB；在普通 Spring 中，具体选择还要看 `proxyTargetClass` 配置和目标类是否提供接口。

**详细原理版**

- JDK 动态代理：`Proxy.newProxyInstance()` 根据目标对象实现的接口生成代理类，代理方法统一进入 `InvocationHandler#invoke()`。Spring 在这里匹配 `Advisor`，组织拦截器链，执行通知后再通过反射调用目标方法。代理对象与目标对象实现相同接口，但不是目标类的子类，因此只能代理接口中暴露的方法。
- CGLIB 代理：Spring 使用 CGLIB 的 `Enhancer` 为目标类生成子类，子类重写可代理方法，并把调用交给 `MethodInterceptor`。它不要求目标类实现接口，适合直接按具体类注入的场景。
- 选择规则：普通 Spring 中，如果明确设置 `proxyTargetClass=true`、没有可代理接口或要求类代理，通常使用 CGLIB；否则可以使用 JDK 动态代理。Spring Boot 默认配置 `spring.aop.proxy-target-class=true`，所以多数 Boot 项目默认看到的是 CGLIB 代理。
- 共同限制：两种方式都依赖“通过代理对象发起调用”。同一个类中使用 `this` 调用另一个增强方法时，调用没有重新经过代理，因此事务、缓存、异步等通知可能不生效。
...
CGLIB 的核心思路是“动态生成子类”。Spring 在运行时为目标类生成代理子类，重写其中可以被覆盖的方法。调用代理方法时会先进入 `MethodInterceptor`，Spring 在这里执行事务、日志等拦截器链，然后再调用目标方法。为了减少普通反射的开销，CGLIB 会为方法建立索引，并通过 `FastClass`、`MethodProxy` 完成快速分派。因为它依赖继承和方法重写，所以 `final` 类不能被代理，`final`、`private`、`static` 方法也不能被增强；此外，自调用仍然绕过 Spring 的代理链。

**详细原理版**

1. **发现增强规则**：Spring 创建 Bean 时，`AbstractAutoProxyCreator` 一类的后置处理器会查找与该 Bean 匹配的 `Advisor`，其中包含切点和通知。
2. **创建代理子类**：如果选择 CGLIB，Spring 通过 `Enhancer` 以目标类为父类生成新的字节码。Spring Framework 已经把 CGLIB 重新打包到 `spring-core` 中，一般不需要单独引入 CGLIB 依赖。
3. **重写目标方法**：生成的子类会重写非 `final` 且可见的方法，并为不同类型的调用配置回调。普通增强方法通常进入 Spring 的 `DynamicAdvisedInterceptor`。
4. **执行拦截器链**：代理收到调用后，根据当前方法查找并按优先级组织匹配的 `Advisor`。各拦截器以嵌套调用的方式执行，返回通知只在正常返回后执行，异常通知只在抛出匹配异常时执行，最终由链尾调用目标方法。
5. **快速方法分派**：CGLIB 会生成 `FastClass`，把方法签名映射为整数索引，再通过索引定位方法，避免每次都走完整的 `Method.invoke()` 反射查找流程。这里优化的是方法分派过程，不改变代理仍需执行拦截器链的事实。
6. **返回代理 Bean**：代理对象替换原始 Bean 暴露到 Spring 容器中。只有调用方拿到并调用这个代理对象，增强逻辑才有机会执行。

CGLIB 的边界也来自这套设计：类被 `final` 修饰时无法生成子类；方法被 `final` 修饰时无法重写；`private` 方法对子类不可见；`static` 方法属于类而不是实例。这些方法即使写了切点或 `@Transactional`，也不会按预期进入 Spring AOP 拦截器链。
...
### 过滤器和拦截器都能拦请求，为何还要分开用

过滤器和拦截器处于不同的执行层级，能获取的信息也不同，因此适合承担不同职责：

- `Filter` 属于 Servlet 规范，由 Servlet 容器管理，执行位置在 `DispatcherServlet` 之前。它能处理映射范围内的 Servlet 请求，但通常不知道请求最终对应哪个 Controller 方法，适合处理跨域、字符编码、请求包装、响应压缩等与具体业务接口无关的通用逻辑。
- `Interceptor` 属于 Spring MVC，执行位置在 `DispatcherServlet` 内部。它不仅能拿到请求和响应，还能获取当前处理器；当处理器是 `HandlerMethod` 时，还可以知道具体的 Controller、方法及其注解，适合登录校验、权限判断、接口审计和耗时统计等需要感知业务接口的逻辑。
...
所以项目中通常把底层、通用、与 Spring MVC 无关的能力放到过滤器中，把需要感知 Controller 方法或注解的接口级控制放到拦截器中。二者不是重复实现，而是在不同层级各司其职。

一句话概括，过滤器拦得更早、范围更广，拦截器离业务更近、控制粒度更细。
...
Spring事务传播定义了 当一个事务方法调用另一个事务方法时（嵌套事务语义），传播行为决定“子事务如何参与父事务”。

以 父事务（外层） 调 子事务（内层） 为主线：

Spring 提供了七种事务传播行为：
1. **REQUIRED**：如果当前存在事务，则子方法加入该事务；如果当前没有事务，则创建一个新的事务。这是最常用的传播行为。
   - 行为：加入当前事务或创建一个新的事务。
   - 适用场景：大多数业务场景，确保方法在一个事务中运行。
2. **REQUIRES_NEW**：无论父事务是否存在，都创建一个新的事务，如果存在父事务，则将父事务挂起。
   - 行为：始终创建一个新事务，并挂起当前事务(如果存在)。
   - 适用场景：审计日志/独立记录失败不影响主事务
...
7. **NESTED**：如果当前存在事务，则在嵌套事务内执行；如果当前没有事务，则与 `REQUIRED` 的效果相同。
...
```
### Spring事务失效场景
在使用 Spring 框架时，事务管理（Spring Transaction）是一个非常重要的功能，但在某些情况下，事务可能会失效。以下是常见的事务失效场景及其原因分析：

**方法未被代理**
原因：Spring 的事务管理依赖于 AOP 代理，如果事务方法没有通过代理调用，事务将不会生效。
示例：
```java
@Transactional
public void methodA() {
    // 事务操作
    methodB(); // 内部调用，不走代理，事务无效
}
...
解决方法：
- 将 methodB() 提取到另一个带有 @Transactional 注解的类中，由代理管理。
- 通过 ApplicationContext 获取当前 Bean 并调用 methodB()。


**事务方法为 private 或 final**
原因：Spring AOP 基于动态代理，代理类无法拦截 private 或 final 方法。
示例：
```java
@Transactional
...
```
解决方法：确保事务方法的访问修饰符为 public。

**异常未被捕获**
原因：Spring 默认只有未捕获的 运行时异常（RuntimeException）或错误（Error）会触发事务回滚。
示例：
```java
@Transactional
public void methodA() {
...
```
解决方法：手动抛出异常或配置事务注解，使其支持检查型异常：
```java
@Transactional(rollbackFor = Exception.class)
```

**嵌套事务传播属性设置错误**
原因：默认传播属性为 Propagation.REQUIRED，如果不正确设置传播属性，可能导致事务嵌套失效或不生效。
示例：
```java
@Transactional
public void methodA() {
    methodB(); // 默认 REQUIRED，参与同一事务
}
...
```
解决方法：根据实际需求设置正确的传播属性（REQUIRED、REQUIRES_NEW、NESTED 等）。

**代理模式不匹配**
原因：Spring AOP 默认使用 JDK 动态代理或 CGLIB。如果使用 JDK 动态代理，事务失效可能是由于目标类未实现接口。
解决方法：确保目标类实现接口，或者强制使用 CGLIB 代理：
```java
@EnableTransactionManagement(proxyTargetClass = true)
```

**未启用事务管理**
原因：如果没有配置事务管理器或未启用事务支持，事务注解无效。
解决方法：确保在配置类中启用了事务管理：@EnableTransactionManagement

**多数据源事务问题**
原因：在多数据源环境下，如果没有配置分布式事务或多个数据源事务管理器，事务可能无法正确回滚。
解决方法：使用 @Primary 指定默认事务管理器。 配置分布式事务管理器（如 Atomikos、Seata）。

**事务拦截器优先级问题**
原因：如果某些切面拦截器优先级高于事务拦截器，可能导致事务未生效。
解决方法：检查 AOP 切面优先级，调整为事务优先级最高。
...
- 是一个复合注解，包含了 `@Configuration`, `@EnableAutoConfiguration` 和 `@ComponentScan` 三个注解。
...
### 6. **@RestController**
- 是 `@Controller` 和 `@ResponseBody` 的组合，通常用于 RESTful Web 服务。
...
### SpringBoot启动流程
1. 调用启动类的`main` 方法
   - Spring Boot 应用通常是通过一个包含 `@SpringBootApplication` 注解的主类启动的，该类的 `main` 方法中调用`SpringApplication.run()`。
...
3. 设置启动配置
   ...
   - 此时，`SpringApplication` 根据类路径、系统属性等配置自动决定应用类型(`Reactive`、`Servlet` 或 `None`)。
...
6. 配置环境 (`Environment`)
   - 在 `ApplicationContext` 准备好后，Spring Boot 开始配置运行时环境。
   - 环境配置包含了系统属性、环境变量以及外部配置文件(如 `application.properties` 或 `application.yml`)，并通过 `ConfigurableEnvironment` 注入到上下文中。
7. 刷新 `ApplicationContext`
   - Spring Boot 调用 `refresh()` 方法，完成 `ApplicationContext` 的初始化，包括 Bean 的创建、依赖注入、生命周期管理等。
   - 在此阶段，所有的 Bean 被完全加载和实例化，`CommandLineRunner` 和 `ApplicationRunner` 也会在此之后执行。
8. 启动内嵌的 Web 服务器(如果有)
   - 如果是 Web 应用，Spring Boot 会在应用上下文加载完成后启动内嵌的 Web 服务器(例如 Tomcat、Jetty 或 Undertow)。
   - 这个步骤通常是通过 `WebServerStartStopLifecycle` 组件来完成的。
9. `ApplicationRunner` 和 `CommandLineRunner` 执行
   - 一旦所有的 Bean 都初始化完毕，Spring Boot 会执行所有实现了 `ApplicationRunner` 或 `CommandLineRunner` 接口的组件。这些组件通常用于执行一些应用启动后的初始化逻辑。
10. 完成启动，进入事件循环
...
1. 启动和初始化
   1. 启动 Tomcat
      - 当 Tomcat 启动时，它会加载并初始化所有配置文件(如 `server.xml`、`web.xml`)。
      - 它会创建并配置一个或多个 Connector 对象，这些对象负责接收来自客户端的 HTTP 请求。
   2. 配置 Connector
       - 在 `server.xml` 文件中配置 Connector，例如：
         ```xml
...
自动装配(Auto-Configuration)是 Spring Boot 的核心能力。它会根据项目中的依赖、容器中已有的 Bean 和配置文件中的属性，判断应用需要哪些组件，并自动将它们注册到 Spring 容器中。例如引入 Web Starter 后，Spring Boot 就可以自动配置 Spring MVC 和内嵌 Tomcat，不再需要开发者手动编写大量配置。

Spring Boot 自动装配的整体流程如下：

1. 启动自动装配
   - 启动类上的 `@SpringBootApplication` 是一个组合注解，主要包含配置`@SpringBootConfiguration`、启动自动配置`@EnableAutoConfiguration` 和组件扫描`@ComponentScan`。
   - `@ComponentScan` 负责扫描业务代码中的 `@Component`、`@Service`、`@Controller` 等组件；`@EnableAutoConfiguration` 负责导入框架提供的自动配置类。两者职责不同，不要混为一谈。
2. 导入自动配置选择器
   - `@EnableAutoConfiguration` 通过 `@Import(AutoConfigurationImportSelector.class)` 引入 `AutoConfigurationImportSelector`。
   - `AutoConfigurationImportSelector` 会收集自动配置候选类，并完成去重、排除和初步过滤。此时加载的是“候选配置”，并不代表它们都会生效。
3. 加载自动配置候选类
   - Spring Boot 2.6 及以前通过 `SpringFactoriesLoader` 读取 `META-INF/spring.factories` 中 `EnableAutoConfiguration` 对应的自动配置类。
   - Spring Boot 2.7 引入并推荐使用 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`，由 `ImportCandidates` 读取其中的自动配置类，同时兼容旧的 `spring.factories` 方式。
   - Spring Boot 3.x 不再支持通过 `spring.factories` 的 `EnableAutoConfiguration` 键注册自动配置，必须使用 `AutoConfiguration.imports`。需要注意，`spring.factories` 的其他用途并没有全部被移除。
4. 根据条件过滤配置
   - 自动配置类通常会使用一系列 `@Conditional` 条件注解，只有满足条件时才会生效。
   - `@ConditionalOnClass`：类路径中存在指定类时生效。例如存在 `DataSource` 类，才考虑配置数据源。
   - `@ConditionalOnBean`：容器中存在指定 Bean 时生效。
   - `@ConditionalOnMissingBean`：容器中不存在指定 Bean 时生效，常用于让 Spring Boot 的默认配置给用户自定义配置让路。
   - `@ConditionalOnProperty`：配置文件中的属性满足要求时生效。
   - `@ConditionalOnWebApplication`：当前应用是指定类型的 Web 应用时生效。
5. 注册 Bean 并绑定配置
   - 条件满足后，自动配置类会作为配置类参与容器刷新，其中通过 `@Bean` 定义的组件会被注册到 IoC 容器。
   - `@EnableConfigurationProperties` 配合 `@ConfigurationProperties`，可以把 `application.properties` 或 `application.yml` 中的配置绑定到属性对象，再用于创建 Bean。
6. 用户配置优先
   - Spring Boot 的自动配置通常带有 `@ConditionalOnMissingBean`。如果用户已经定义了同类型 Bean，默认配置就不会再创建，这也是“约定优于配置”的体现。
   - 用户还可以通过配置文件覆盖默认属性，或者使用 `@SpringBootApplication(exclude = ...)`、`spring.autoconfigure.exclude` 排除指定的自动配置类。
7. 完成容器初始化
   - 当 `SpringApplication.run()` 推进到刷新 `ApplicationContext` 时，筛选后的自动配置类会和普通配置类一起完成 Bean 定义注册、实例化和依赖注入。
   - 如果需要排查某个自动配置为什么生效或没有生效，可以使用 `--debug` 或设置 `debug=true` 查看条件评估报告(Condition Evaluation Report)。

一言以蔽之，Spring Boot 自动装配就是：`@EnableAutoConfiguration` 导入候选配置类，条件注解决定配置是否生效，满足条件后注册 Bean，并在用户已经自定义 Bean 时主动让路。

### 如何自定义一个Spring Boot Starter✅
Starter 的作用是让业务项目引入一个依赖，就能获得相关依赖和默认配置，让代码快速跨应用复用。

自定义 Starter 的核心可以分成两部分：Starter 模块负责聚合依赖，AutoConfiguration 模块负责根据条件自动创建 Bean。下面以一个问候服务 `greeting-spring-boot-starter` 为例。

1. 设计模块结构
   - 标准做法通常拆成 `greeting-spring-boot-starter` 和 `greeting-spring-boot-starter-autoconfigure` 两个模块。
   - `starter` 模块只负责聚合依赖；`autoconfigure` 模块存放业务组件、配置属性和自动配置类。
   - 如果只是公司内部使用的小组件，也可以把两部分合并成一个模块，不必为了形式强行拆分。
2. 定义配置属性和业务组件
   - 使用 `@ConfigurationProperties` 接收业务项目中的配置，再把配置传给真正提供能力的组件。
   ...
   public class GreetingService {
       ...
       public GreetingService(String prefix) { this.prefix = prefix; }
       public String greet(String name) { return prefix + ", " + name; }
   }
   ```

3. 编写自动配置类
   - 使用 `@AutoConfiguration` 声明自动配置类，使用 `@EnableConfigurationProperties` 注册配置属性。
   - `@ConditionalOnProperty` 控制功能开关，`@ConditionalOnMissingBean` 保证用户自己提供 Bean 时，默认实现会主动让路。

   ```java
   @AutoConfiguration
   @EnableConfigurationProperties(GreetingProperties.class)
   @ConditionalOnProperty(prefix = "demo.greeting", name = "enabled",
...
4. 注册自动配置类
   - Spring Boot 2.6 及以前主要使用 `META-INF/spring.factories`：
   ...
   - 这时自动配置类通常使用 `@Configuration(proxyBeanMethods = false)`，因为 `@AutoConfiguration` 是 Spring Boot 2.7 才引入的。
   - Spring Boot 2.7 同时支持 `spring.factories` 和新的 `AutoConfiguration.imports`，推荐逐步迁移到新方式。
   - Spring Boot 3.x 不再支持通过 `spring.factories` 的 `EnableAutoConfiguration` 键注册自动配置，必须创建：

   ```text
   META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports
   ```

   - 文件内容是自动配置类的全限定名，每行一个：
...
5. 聚合 Starter 依赖
   - `starter` 模块依赖 `autoconfigure` 模块和使用方真正需要的第三方库，本身通常不写业务代码。
   - `autoconfigure` 模块至少需要依赖 `spring-boot-autoconfigure`。`spring-boot-configuration-processor` 只用于生成 IDE 配置提示，不是自动装配生效的必要条件。
6. 引入并验证 Starter
   - 业务项目先通过 Maven 引入 `greeting-spring-boot-starter`：
   ...
   - 引入后自动配置默认生效，可以直接注入 `GreetingService`。如果需要修改默认前缀或关闭功能，再添加配置：
   ...
   - 测试时至少验证三种情况：默认配置能够创建 Bean；`enabled=false` 时不创建 Bean；用户自定义 `GreetingService` 后，默认 Bean 会让路。

一言以蔽之，自定义 Starter 就是：定义配置和业务组件，编写带条件的自动配置类，在 `AutoConfiguration.imports` 中完成注册，最后用 Starter 模块把依赖聚合起来。

#### 自定义Starter的目的

在多个 Java 应用共享同一业务域公共能力的场景中，自定义 Starter 主要解决四个组织级问题：

1. 跨应用复用
    - 审计日志、日志脱敏等能力会被多个应用重复使用。没有 Starter 时只能复制代码；有了 Starter，应用引入一个依赖坐标即可接入。
    - 例如审计自动配置类可以把拦截器注册到持久层相关组件中，业务应用不需要重复编写接入代码。
2. 统一技术标准
    - 同一业务域的安全规范、异常体系和校验规则需要保持一致。Starter 可以把规范固化成代码，从“文档要求大家遵守”变成“引入后默认生效”。
3. 分离业务与基础设施
    - 业务应用只关注订单、结算等核心业务，审计、脱敏、审批等横切能力下沉到 Starter，由基础能力团队统一维护。
4. 集中演进
    - 安全漏洞修复或公共能力升级时，只需修改 Starter 并推动应用升级版本，不需要在每个应用中重复修改。
    - 同时配合 `@ConditionalOnMissingBean` 保留扩展点，个别应用需要定制时可以覆盖默认实现，不必 fork 整套代码。

#### 为什么一个项目中会有多个Starter

技术上当然可以把所有能力塞进一个 Starter，但会立刻付出四笔代价：

1. 依赖绑架
    - Starter 会把声明的依赖传递给使用方。例如审计 Starter 可能继续传递持久层、任务调度和基础工具等依赖。
    - 如果所有能力都放进一个 Starter，应用哪怕只想使用一个工具类，也可能被迫引入大数据 SDK、流程客户端、缓存客户端等大量无关依赖。
    - 最终会导致构建产物膨胀、自动配置评估增多、依赖冲突概率上升，安全审计面也随之扩大。
2. 发布节奏互相锁死
    - 如果所有能力共用一个制品，修改一行审计代码也要发布整个 Starter，所有使用方都需要重新升级和回归。
    - 拆分后，升级审计 Starter 只影响真正依赖它的应用，其他应用不需要被迫跟随。
3. 故障半径扩大
    - 大 Starter 中任意一个能力出现启动期 Bug，都可能阻塞所有应用启动，即使这些应用根本没有使用该能力。
    - 按能力域拆分后，问题会被限制在引用了对应 Starter 的应用中。
4. 无法按需组合
    - 每个 Starter 对应一个横切能力域，使用方可以像搭积木一样组合：

   ```text
   核心业务应用 = 基础工具 + 安全 + 审计 + 流程 + 数据集成
   管理后台应用 = 基础工具 + 安全
   其他业务应用 = 根据需要选择能力
   ```

    - Starter 之间也可以分层依赖，例如审计 Starter 依赖任务调度 Starter 和基础工具 Starter。如果合成一个大 Starter，就失去了这种组合弹性。

一句话概括，一个大 Starter 也能把功能做出来，但多个 Starter 才能做到“用多少拿多少、改谁发谁、坏哪隔哪”。
...
- **核心组件**：
    ...
    - Starter模块：提供常用的依赖集合，如`spring-boot-starter-web`用于Web开发，`spring-boot-starter-data-jpa`用于JPA持久层开发
    - Actuator：提供监控和管理功能
...
### 总结：
- **Spring Boot**关注的是简化单个服务的开发，而**Spring Cloud**关注的是简化微服务架构的分布式管理。
