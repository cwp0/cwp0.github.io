...
6. `SliceHelper.reRankAndSelect` 应用层二次 Rerank：`原召回分 × 0.7 + 文档名 rerank 分 × 0.3`(`qwen3-rerank` 模型)加权，降序选择有权限切片；`knowledgeSliceTopK` 代码默认值为 20，当前 Diamond 配置为 10，因此最终最多取 Top-10 入模；
...
- 汇总完所有知识源的切片后，再做一次应用层的二次`rerank`(`原召回分 × 0.7 + 文档名 rerank 分 × 0.3`(`qwen3-rerank` 模型)加权，降序选择有权限切片；`knowledgeSliceTopK` 代码默认值为 20，当前 Diamond 配置为 10，因此最终最多取 Top-10 入模)，然后组装一份多角色的提示词——核心指令用`role`-`system`、当前时间和用户身份信息用`role`-`user`单独传、历史对话原样保留、最后把选中的知识切片也用`role`-`tool`角色单独传(不同角色分开传是为了让模型更容易分清哪些是指令、哪些是背景信息、哪些是检索结果)，交给模型流式生成答案，返回一条“热的”结果流。
...
- **① 路由与检索并行化**：原来工具路由、Query Rewrite、语言检测、知识库分类、政策制度分类是串行的，改成 5 路 CompletableFuture 并行，其中 Rewrite 内部又套了 3 路并行，相当于 8 个任务、7 次 LLM 调用同时跑。路由使用独立的 `AI_ROUTING_POOL`；进入 RAG 后，再分别使用 `RAG_STRATEGY_POOL`、`RAG_RETRIEVE_POOL` 和 `KNOWLEDGE_AUTH_POOL`，也就是整体有 4 个相关线程池、RAG 内部有 3 层，避免嵌套异步导致线程饥饿。
- **② 端到端流式推送**：用 RxJava ReplayProcessor 做桥梁——上游接百炼 Flowable 流式返回；Web 端通过 Dubbo Triple StreamObserver 逐帧推送，钉钉端则按 300ms 窗口合并增量后刷新卡片，避免每个 token 都调用一次卡片 OpenAPI。
- **③ 1.5 秒中间态提示**：完成路由并进入知识问答后，先发送占位卡片，再用 TaskScheduler 注册 1.5 秒延迟任务；如果模型还没返回首批正文，就推"正在搜索知识库..."提示，消除召回和生成阶段的空白等待。它不覆盖路由阶段的卡顿，因此属于体验优化，不会降低真实 TTFT。
- **④ 降级保可用**：路由解析异常或 operationId 无执行器时直接走默认知识问答；钉钉 API 限流时随机退避 1～2 秒重试；百炼旧链路在智能体 AppId 未配置时选择工作流 AppId。这一项解决的是失败和可用性问题，不是性能优化，而且 AppId 选择只是配置级回退，不是运行故障后的自动切换。

### 首字响应延迟优化前后是多少?性能瓶颈主要在哪里

**先说结论：**如果面试时需要给出一组合理的量化口径，可以回答“优化前首批响应 P50 约 5～6 秒、P95 约 8～10 秒；优化后 P50 约 3 秒、P95 约 5 秒”。其中最明显的是路由阶段：从串行累计约 2 秒，降低为并行等待最慢任务的约 500ms。这里的数字属于结合链路耗时给出的合理面试估算，不是当前源码仓库中可查到的线上实测报告，真实值应以生产埋点重新统计。

统计时，我会把首字口径定义为：

```text
服务端近似首批响应耗时 = KQA_FIRST_CHAR_DISPLAYED - START_TIME
```

它不能完全等同于模型原生 TTFT 或用户真正看到首字的时间：钉钉链路会先按 300ms 窗口合并模型帧，埋点又发生在卡片更新请求之前，因此既包含服务端缓冲，也不包含钉钉网络传输和客户端渲染。更严谨的做法是按版本分别统计 P50、P95 和 P99，而不是只报平均值。

当前性能瓶颈主要有三类：

1. **最大瓶颈是生成前的多源 RAG 慢尾。**7 个 Strategy 虽然并发执行，但聚合时仍要等待最慢的一路；每个 Strategy 内部还会发起“知识库 × Query”多次检索，随后串接百炼 Rerank、原系统鉴权和应用层文档名 Rerank。整个阶段结束后才能调用回答模型，所以任意一个知识源、鉴权接口或 Rerank 变慢，都会直接推高首字延迟。
2. **第二个瓶颈是路由阶段的最慢任务。**并行化把耗时模型从 `T1+T2+...+Tn` 降成 `max(T1,T2,...,Tn)`，但 `allOf().join()` 仍然要等待最慢的一路；Query Rewrite 外层任务还会在路由池中等待 3 个子任务，高并发时仍可能产生排队和线程饥饿。
3. **第三个瓶颈是回答模型首 Token。**RAG 完成后才会组装对话历史、用户信息和 Top-10 知识切片并调用模型。Prompt 越长，Prefill 越慢；模型平台负载和网络波动也会影响首 Token。流式输出只能缩短“完整答案等待时间”，不能消除模型产生首 Token 之前的耗时。

**最后总结：**这次优化本质上做了两件事：用并行化缩短真实关键路径，用流式输出和 1.5 秒提示改善用户感知。优化后路由已经不是最大的耗时来源，后续应重点治理多源 RAG 的慢尾、补齐 Strategy 和检索调用的超时隔离，并持续压缩最终 Prompt 的长度。

> **口径自查：**仓库已经记录 `START_TIME`、`TOOL_ROUTED`、`KQA_RAG_STARTED`、`KQA_RAG_COMPLETED`、`KQA_FIRST_CHAR_DISPLAYED` 等时间点，但没有现成的生产 P50/P95 报表，也没有客户端真实渲染时间。因此上面的 5～6 秒、3 秒等数字应在面试前通过线上 `ai_chat_log.question_extend_info.timeRecord` 重新统计后替换。
...
源码只能确认调用链路，不能给出可信的平均 Token 或月成本。生产代码没有持久化每次调用的 input/output/reasoning Token，也没有按模型汇总费用；因此“单次多少 Token”和“月成本多少”目前都是**待确认**，不能根据 Prompt 字符数直接拍一个数字。

一次知识问答可能包含技能路由、语言识别、三路 Query Rewrite、知识库标签识别、政策分类、Rerank、答案生成和来源抽取；命中兜底时还会增加通识问答，跨语种时增加翻译，在线打标又是独立的异步模型调用。不同分支的调用次数、上下文长度和输出长度差异很大，所以平均成本必须按真实调用明细统计。

核实方法是从百炼对应 API Key 或 Workspace 导出按模型区分的输入 Token、输出 Token、缓存命中和调用次数，再按当期模型单价汇总账单；同时按 `ai_chat_log` 的有效知识问答 PV 计算“总 Token/PV”和“总费用/PV”。月成本则按各模型 `输入Token×输入单价 + 输出Token×输出单价` 求和，再加 Rerank、Embedding 等独立计费项。面试中的稳妥回答是：“代码侧能解释调用扇出和优化方向，准确数字以百炼用量与财务账单为准，当前仓库没有证据支撑具体金额。”

### 菜小蜜Agent在对话中出现异常是怎么处理的

我没有在最外层粗暴地包一个 `try-catch`，而是按故障域拆成六层处理，原则是：**能隔离就隔离，能降级就降级；涉及权限时宁可少答，不能错给。**
...
**先说结论：** 菜小蜜不是只有一个 Top K，而是采用“**Top-100 粗召回 → Top-20 重排候选 → Top-10 最终入模**”的三层漏斗。前两层负责保证召回率和过滤弱相关内容，最后一层控制真正交给大模型的上下文规模，整体原则是“宽召回、严排序、窄入模”。
...
如果面试官追问“为什么不更小”，可以回答：粗召回 K 过小会让正确切片在 Rerank 前就被截断，最终入模 K 过小则可能让长文档或跨章节问题缺少必要证据。如果追问“为什么不更大”，可以回答：每个知识库和 Query 都会独立返回候选，多路汇总会放大鉴权、排序和 Token 成本，而且尾部切片相关性通常更弱，继续增大 K 的边际收益会下降。这些判断仍应通过评测曲线验证，不能只凭经验下结论。

线上调整方面，当前可以通过 Diamond 修改最终入模的 `knowledgeSliceTopK`，支持不改代码地调整和回滚；`denseSimilarityTopK=100` 与 `rerankTopN=20` 当前仍写在检索代码中，不能表述为三层参数都已支持动态调整。如果后续按问题复杂度自适应，FAQ 可以使用较小 K，长文档和多跳问题可以使用较大 K 并配合重排和 Token 上限，但这属于优化方向，不是当前已实现能力。客服 FAQ 的 5～10、企业文档的 20、合同财报的 30～50、代码问答的 50 和多跳推理的 30 以上，只能作为通用调参起点，不能冒充菜小蜜已经验证过的线上结论。

**最后总结：**菜小蜜当前是 Top-100 保召回、Top-20 做重排候选、Top-10 控制最终入模，体现的是“宽进严出”。参数现状可以从代码和配置确认，但最优性仍需用固定评测集、K 曲线和线上灰度数据验证。
...
**先说结论：** 菜小蜜没有把所有知识塞进同一个向量库，而是按照知识来源、审核状态和权限模型拆成 7 个检索 Strategy、6 类逻辑知识源。应用侧负责扫描、导出和上传完整文档，百炼平台负责文档解析、智能切片、Embedding 和内置向量存储，整体是一条“**扫描变更 → 抽取内容 → 上传百炼 → 智能切片 → 向量化索引 → 增量治理**”的链路。
...
需要注意：百炼页面中的“最终召回最大数量 5”属于平台原检索参数配置；菜小蜜当前调用 Retrieve 接口时会显式传入 Top-100、0.2 和 Top-20，多路结果汇总后应用层再选择 Top-10 入模，因此运行链路仍以请求级代码参数为准。

**最后总结：** 菜小蜜的知识库建设不是一次性上传，而是“多知识源分库、完整文档同步、百炼智能切片、标签粗筛、原系统精鉴权、持续评测治理”的完整闭环。不过它不能保证源知识天然正确：源文档写错时，RAG 只会更忠实地回答错误内容；跨来源的新旧冲突目前也没有完整的“更新时间 + 权威度”确定性仲裁，仍需要内容 Owner、审核发布和错误回溯修订机制兜底。

### 菜小蜜的知识切片设计原则是什么

**先说结论：** 菜小蜜的切片原则是“**语义完整、粒度适中、结构保留、来源可追溯、标签可过滤、内容可治理**”。Java 应用上传的是完整文件，真正的 Chunk 由百炼智能切分生成，不是应用层每隔固定字符机械截断。
...
**最后总结：**切片设计不是“越小越准”或“越大信息越全”，而是在语义完整、召回精度和上下文成本之间取平衡；菜小蜜通过智能切分和 600 字符上限控制基础粒度，再通过标题、Metadata、标签、精鉴权和 Rerank 保证最终可用性。

### 菜小蜜的知识切片中包含什么参数以及标签

**先说结论：**这个问题必须区分三层：上传时是“文件参数和文件标签”，百炼生成后是“切片基础字段”，召回后还会追加“鉴权和重排字段”。把三层混在一起，就容易把运行时得分误说成入库 Metadata，或者把文件标签误说成 `KnowledgeSlice` 的成员变量。
...
**最后总结：**入库时主要保存文档、解析器、类目和文件标签；百炼切片包含 ID、标题、正文和来源；应用召回后再补充 Query、权限、链接和重排结果。真正进入 Prompt 的，只是完成去重、精鉴权、综合排序并进入 Top-10 的切片。

### Rerank的0.7/0.3权重是怎么确定的

源码能确认 `SliceHelper` 使用“原召回分 × 0.7 + 文档名 Rerank 分 × 0.3”，但仓库里没有参数实验、评测报告或提交记录能证明这组权重来自网格搜索。因此面试时不能虚构“比较过四组参数后 0.7/0.3 最优”。

合理的确定方式应该是准备带相关文档标注的 Query 集合，对多组权重分别计算 Recall@K、MRR/NDCG 和最终答案准确率，再结合线上 bad case 灰度选择。直觉上，内容分作为主信号，文档名分作为辅助信号，可以避免标题相似但正文无关的文档被抬得过高；但 0.7/0.3 的实际依据仍需从历史评测或变更记录补证。
...
**先说结论：**从理想架构看，权限校验最好直接发生在检索层，让向量库只召回当前用户有权查看的切片。菜小蜜现在采用“**检索前标签粗过滤 + 召回后原系统精鉴权**”，并不是因为后置鉴权更优，而是在多套内部知识库权限模型难以统一、ACL 下推和同步改造成本较高的情况下做出的工程折中。
...
**最后总结：**最优方向是把可靠的用户级 ACL 下推到检索层，做到“无权限的数据根本不召回”；当前实现则遵循“**能前置的先粗筛、无法统一的交给权威系统精鉴权、权限判断失败时宁可少答也不能越权**”。所以，召回后鉴权不是理论上的最优解，而是在内部知识源异构和权限改造成本约束下，兼顾落地成本、权限准确性与安全性的折中方案。
...
### 你怎么知道回答是准确的?怎么验证
结论先说：当前主要靠“在线真实流量打标 + 离线标准 Q&A 评测”两套机制验证，用户反馈和人工结果作为纠偏信号；源码没有证明固定比例人工复核已经落地。

在线链路覆盖真实用户问题。SchedulerX 先把最近 1 天聊天记录同步到 `message_mark`，再默认扫描最近 30 天的待打标记录；`MessageAiMarker` 每轮按时间顺序最多取 10 条，每条独立调用一次 `qwen3.7-plus`。离线链路覆盖标准问题，旧任务从 `cxm_answer_score` 取标准问题，让菜小蜜重新回答，再由 `deepseek-v3` 比较标准答案和 AI 答案并给出单条分数。

这两套机制互补：在线数据分布真实，但自动 Judge 和统计分母都有偏差；离线问题有标准答案，适合定向回归，但题集更新和汇总能力不足。因此面试时不能只说“准确率 85%”，而要继续交代这是哪个数据集、哪个时间窗口、谁打的标签以及分子分母是什么。

### 在线打标具体怎么做
当前 `MessageAiMarker` 是单条打标模式，具体流程是：

1. 每轮最多读取 10 条待打标记录，逐条提交 `AI_MARK_POOL`，单条失败不影响其他记录；
2. 通过 `chatLogId` 找到 `platformSessionId`，只取同会话、当前记录之前最近 5 条问答作为上下文，避免把不同会话串在一起；
3. 输入由“历史问答 + 当前问题和回答 + 参考知识”组成。参考知识来自本次回答真实使用的 `knowledge_slices`，只取 `selectedForPrompt=true` 的切片，每条最多保留 500 字符；
4. 调 `qwen3.7-plus`，期望返回单个 JSON：`{"markResult": true|false, "reason": "..."}`。解析失败时本轮不更新记录，让它留在待打标池等待下轮重试；
5. LLM 结论落库前再执行确定性覆盖：`moduleCode=1001` 的问答卡片记 SUCCESS，`output` 是 JSON 且 `success=true` 记 SUCCESS，最后用户反馈 USEFUL/USELESS 再覆盖为 SUCCESS/FAIL；
6. 最终结果优先使用人工 `result`，人工尚未给终态时才使用 `ai_mark_result`。本轮自动打标判 FAIL 会把 `follow_status` 从 INIT 推到 PROCESSING；同一流程最终判为 SUCCESS 时，PROCESSING 可恢复为 INIT。

因此完整优先级是：**人工结果 > 用户反馈 > `output.success` > 问答卡片规则 > LLM 判断**。自动化负责扩大覆盖面，用户反馈和人工结果负责纠偏，不能把模型自评当作绝对真值。

### 离线评测具体怎么做
旧离线链路由 `CxmAnswerScoreProcessor` 触发，默认参数是 `timeLine=now-4h`、`accuracyRateLine=0.85`、`countLimit=200`。SQL 会选择从未评过、评分失败或者分数低于 0.85 的记录，按更新时间和 ID 排序，每轮最多处理 200 条。

评测时使用 5 线程固定池。每条记录先拿 `xl_question` 调菜小蜜得到 `ai_answer`，再用 `PromptEvaluateAnswerScore` 把 `xl_answer` 和 `ai_answer` 拼成裁判 Prompt，调用百炼默认的 `deepseek-v3`，最后把模型输出直接 `new BigDecimal(score)` 写入 `accuracy_rate`。标准 Q&A 的采集代码会从云灵拉 ONLINE 状态知识，答案优先级是“机器人个性化答案 > 通用答案 > 知识点自带答案”，空答案或超过 3 万字符的答案会跳过。

这里的 0.85 只是**低分重评队列的筛选阈值**，不是源码已经计算出的“整体准确率 85%”。高于或等于 0.85 的记录会退出队列，之后 Prompt 或知识发生回归时不会自动复评。

> **落地自查**：`collectQAFromXl()` 在当前 Processor 中已被注释，定时任务只评已有存量；裁判 Prompt 只接收标准答案和 AI 答案，没有传原问题；分数没有 JSON Schema、0～1 区间校验和 clamp；仓库也没有把 `accuracy_rate` 汇总成离线达标率的代码。要讲整体离线准确率，仍需补固定金标集、版本化全量回归和汇总报表。
...
### 准确率的分子分母到底是什么
线上周报的源码口径是：`PV=COUNT(id)`，`failCount` 是“人工 `result='FAIL'`，或者人工尚未给 SUCCESS/FAIL/INVALID 结论且 `ai_mark_result='FAIL'`”，最终准确率为 `(PV-failCount)/PV`。SQL 只统计 `module_code='101'` 的知识问答，并按渠道、业务域和时间窗口过滤。

这个口径有一个明显偏差：INIT 和 INVALID 记录会进入 PV，但通常不进入 failCount，相当于被默认算成正确，结果会偏乐观。更严谨的线上口径应把无效问题和仍未完成打标的记录排除出分母，并同时披露自动打标覆盖率与人工抽检一致率。

离线侧目前只有每条记录的 `accuracy_rate`，0.85 只是任务重评阈值，仓库没有“达标条数/总条数”的汇总实现。所以在线周报准确率和离线单条分数是两套不相通的指标，不能混成一个数字。
...
`gmt_modified < now-4h` 会让成功走到 `updateScoreResult()` 的空答案、空分或低分记录先退出当前窗口，配合更新时间排序减少同一批记录在短时间内反复消耗裁判调用；若调用直接抛异常，或分数在 `new BigDecimal(score)` 时解析失败，更新时间不会刷新，仍可能在下轮立即被选中。另一个副作用是高分记录不会重新入队，这套任务不能替代固定题集的全量回归。
