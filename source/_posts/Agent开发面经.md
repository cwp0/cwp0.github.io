...
1. **在线验证**。系统对真实用户问答做自动 Judge，判断回答是否与本次使用的知识切片一致，并保存 SUCCESS/FAIL 和原因。这一层覆盖真实问题分布，适合发现线上 bad case，但会受到 Judge 偏差和召回材料质量的影响。
...
在线打标就是在用户已经收到回答后，异步把“问题、回答、对话上下文和本次实际使用的知识”交给 Judge 判断，再用确定性规则、用户反馈和人工结果覆盖模型结论。它不阻塞用户主链路。

具体分为五步:

1. **选待评记录。** SchedulerX 先把符合条件的线上聊天记录同步到 `message_mark`；打标任务默认扫描最近 30 天尚无 SUCCESS/FAIL/INVALID 终态的记录，每轮按时间顺序最多取 10 条，并逐条提交到 `AI_MARK_POOL`，某一条失败不会影响其他记录。
2. **还原答题现场。** 系统通过 `chatLogId` 找到会话，只取同一员工、同一会话、当前记录之前最近 5 条问答作为上下文；同时带上当前问题、当前回答，以及本次真正进入 Prompt 的知识切片。每条知识切片最多截取 500 字符，避免把无关候选或超长内容交给 Judge。
3. **调用模型判定。** 使用 `qwen3.7-plus`，要求只返回 `markResult=true/false` 和 `reason`。若模型输出无法解析，本轮不写结果，记录保留到后续任务重试。
4. **执行确定性纠偏。** 问答卡片、结构化 `output.success=true`、用户 USEFUL/USELESS 反馈都可以覆盖 LLM 判断，避免本来已有明确业务结果时仍让 Judge 猜测。
5. **保存并进入运营闭环。** 自动结果写入 `ai_mark_result`；FAIL 记录可进入跟进状态。若后续有人工 `result`，统计时优先采用人工结论。

最终判定优先级可以概括为:**人工结果 > 用户反馈 > 确定性业务结果 > LLM Judge**。所以在线 Judge 的作用是低成本覆盖大量真实流量和发现疑似 bad case，而不是充当绝对真值。
