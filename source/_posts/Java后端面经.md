...
### 内存溢出/内存泄漏区别
- 内存溢出：JVM 需要分配内存时，目标内存区域没有足够空间且无法继续扩展，最终抛出 `OutOfMemoryError`。它可能发生在 Java 堆、元空间、直接内存或本地线程等区域。
- 内存泄漏：对象在业务上已经不再使用，但仍然存在从 GC Roots 到该对象的强引用链，导致 GC 无法回收，长期积累后可能引发 OOM。

二者的关系是：内存泄漏可能导致内存溢出，但内存溢出不一定由泄漏引起。堆设置过小、突发流量导致对象分配过快、大对象申请失败、线程数量过多等情况，即使没有泄漏也可能发生 OOM。

> 递归过深通常抛出的是 `StackOverflowError`，不属于 `OutOfMemoryError`。Java 中已经不可达的对象可以被 GC 回收，不能把“对象不可达”描述为内存泄漏。
...
- 非公平锁：锁被释放之后，允许新线程插队竞争；后申请的线程可能会先获取到锁，竞争失败后通常仍进入等待队列。性能更好，但可能会导致某些线程永远无法获取到锁。
...
- 分段锁(Segmented Lock)
    - 分段锁是一种细粒度锁机制，将资源分为多个段，每个段配有独立的锁，以提高并发性。Java 1.7中的`ConcurrentHashMap`实现就使用了分段锁来提高并发效率。

#### Java中有哪些锁
Java 中常见的锁可以分为 JVM 内置锁和 JUC 提供的显式锁：

- `synchronized`：JVM 内置的可重入独占锁。线程进入同步方法或同步代码块前，需要先获取指定对象的监视器锁；退出同步区域或发生异常时，JVM 会自动释放锁。
- `ReentrantLock`：`Lock` 接口的可重入独占锁，需要在 `finally` 中手动释放。相比 `synchronized`，它还支持可中断获取、超时获取、公平锁以及多个 `Condition` 条件队列。
- `ReentrantReadWriteLock`：可重入读写锁，读锁是共享锁，写锁是独占锁。读读可以并发，读写和写写互斥，适合读多写少的场景。
- `StampedLock`：Java 8 引入的锁，提供写锁、悲观读锁和乐观读三种模式。乐观读不会真正加读锁，而是读取后通过 `validate(stamp)` 判断期间是否有写锁被获取；它适合读多写少的场景，但不支持可重入。

选择时，普通互斥场景优先使用 `synchronized`；需要中断、超时、公平策略或多个条件队列时使用 `ReentrantLock`；读多写少时可以考虑 `ReentrantReadWriteLock`，竞争更低且能够接受重试逻辑时可以考虑 `StampedLock`。

#### Java中有哪些并发同步工具
JUC 还提供了多种线程同步与协作工具，它们解决的是线程之间的等待、汇合或并发限流问题，并不都属于锁：

- `Semaphore`：通过许可证控制同时访问资源的线程数量，常用于限流和连接池等场景。它不强调线程所有权，获取许可证和释放许可证可以由不同线程完成。
- `CountDownLatch`：一个或多个线程调用 `await()` 等待计数器归零，其他线程完成任务后调用 `countDown()`。适合主线程等待多个子任务完成，只能使用一次。
- `CyclicBarrier`：一组线程到达共同屏障后再一起继续执行，适合多个线程相互等待的分阶段任务；屏障触发后可以重复使用。
- `Phaser`：功能更灵活的多阶段同步器，支持线程动态注册和退出，适合参与者数量可能变化的多阶段任务。
- `Exchanger`：让两个线程在同步点交换数据，双方都到达后才能继续执行。

选择时，限制并发数量使用 `Semaphore`；一个线程等待多个任务完成使用 `CountDownLatch`；多个线程互相等待并重复汇合使用 `CyclicBarrier`；需要动态参与者和多阶段协作使用 `Phaser`；两个线程交换数据使用 `Exchanger`。

> `AtomicInteger`等属于基于 CAS 的无锁原子类，`LockSupport`提供 `park()`和`unpark()`阻塞唤醒原语，它们也不属于锁或高层线程协作器。
...
#### synchronized锁升级原理
锁升级特指 HotSpot JVM 对 `synchronized` 的实现优化，对象会根据竞争程度经历无锁、偏向锁、轻量级锁和重量级锁：没有竞争时尽量避免同步开销；出现少量竞争时通过 CAS 和短暂自旋获取锁；竞争持续加剧后，锁会膨胀为重量级监视器，未获取锁的线程进入阻塞等待，以避免持续占用 CPU。

需要注意，偏向锁从 JDK 15 开始默认禁用，因此现代 JDK 通常重点关注“无锁 → 轻量级锁 → 重量级锁”。具体实现会随 JVM 和 JDK 版本变化。

1. 无锁：对象尚未被线程持有，多个线程都可以尝试获取锁。
2. 偏向锁：在传统 HotSpot 实现中，如果锁长期只有一个线程使用，JVM 会让对象偏向该线程，后续进入同步块时减少 CAS 操作。其他线程竞争时需要撤销偏向；该机制从 JDK 15 开始默认禁用。
3. 轻量级锁：线程在栈帧中创建锁记录，通过 CAS 尝试获取锁。竞争较轻、持锁时间较短时，可以配合自旋避免线程阻塞和操作系统调度开销。
4. 重量级锁：如果竞争持续或自旋失败，锁会膨胀为基于 `ObjectMonitor` 的重量级锁，未获取锁的线程可能被挂起，等待持锁线程释放后再被唤醒。

所谓“锁只能升级、不能降级”是面试中的简化说法，主要描述一次竞争过程中的升级方向；对象空闲后，HotSpot 仍可能在安全条件下回收或收缩已经膨胀的监视器，具体行为取决于 JVM 版本和实现。

#### 公平锁/非公平锁
- 公平锁：锁被释放之后，先申请的线程先得到锁。性能较差一些，因为公平锁为了保证时间上的绝对顺序，上下文切换更频繁。
- 非公平锁：锁被释放之后，后申请的线程可能会先获取到锁，是随机或者按照其他优先级排序的。性能更好，但可能会导致某些线程永远无法获取到锁。

`ReentrantLock`默认使用非公平锁，也可以通过构造参数创建公平锁：
```java
ReentrantLock nonFairLock = new ReentrantLock();    // 非公平锁
ReentrantLock fairLock = new ReentrantLock(true);  // 公平锁
```

`synchronized`不能指定公平策略，通常视为非公平锁。需要注意，公平性描述的是锁的竞争策略，并不代表线程一定按照绝对时间顺序执行；即使使用公平的 `ReentrantLock`，无参 `tryLock()`也可以直接抢锁，不遵守公平队列。


#### 可中断锁/不可中断锁
- 可中断锁：线程等待获取锁时，可以响应中断并放弃等待。例如调用 `ReentrantLock.lockInterruptibly()`，收到中断后会抛出 `InterruptedException`。
- 不可中断锁：线程等待获取锁时，不能通过中断退出等待。例如 `synchronized` 的锁竞争和 `ReentrantLock.lock()`；线程通常要等到成功获取锁后才能继续处理。

这里的“可中断”特指等待获取锁的过程是否可以被中断，不代表线程进入同步代码后不能响应中断。

#### 共享锁/独占锁
- 共享锁：一把锁可以被多个线程同时获得。例如 `ReentrantReadWriteLock` 的读锁，多个线程可以同时持有读锁并读取共享数据。
- 独占锁：一把锁同一时刻只能被一个线程持有。例如 `synchronized`、`ReentrantLock`，以及 `ReentrantReadWriteLock` 的写锁。

以 `ReentrantReadWriteLock` 为例：读锁与读锁可以共享；写锁与其他读锁或写锁互斥。

#### 读锁能升级为写锁吗
以 `ReentrantReadWriteLock` 为例，它支持写锁降级为读锁，但不支持直接将读锁升级为写锁。

- 持有读锁时调用 `writeLock().tryLock()`，通常会立即获取失败。
- 持有读锁时调用 `writeLock().lock()`，线程会等待其他读锁释放；由于当前线程自己仍持有读锁，可能形成自我死锁。
- 持有写锁的线程可以继续获取读锁，然后先释放写锁、再释放读锁，从而完成锁降级。

读锁不能直接升级，主要是为了避免多个读线程同时升级时互相等待。例如线程 A、B 都持有读锁并申请写锁，A 等待 B 释放读锁，B 又等待 A 释放读锁，最终形成死锁。

如果业务确实需要从读变为写，应先释放读锁，再获取写锁，并在获得写锁后重新检查共享数据，因为两次加锁之间数据可能已被其他线程修改。
...
- 实现方式上的区别：
   - `synchronized` 通过对象监视器实现锁
   - `ReentrantLock`通过内部的 `Sync`继承 AQS，通过 `volatile state`表示锁的持有和重入次数，使用 CAS 修改状态，并通过 AQS 等待队列和 `LockSupport.park()/unpark()`管理获取锁失败的线程。
- 加锁释放锁：由于`synchronized` 是 JVM 实现的 而`ReentrantLock` 是 JDK 实现的，所以二者加锁和释放锁也有区别。
   - `synchronized`的加锁和释放由 JVM 自动完成；
   - `ReentrantLock`通过 Java API 显式调用 `lock()`和`unlock()`，需要在 `finally`中手动释放。
...
`ReentrantReadWriteLock` 是一个可重入的读写锁，既可以保证多个线程同时读的效率，同时又可以保证有写入操作时的线程安全。
...
```java
/**
 * 基于 synchronized、wait 和 notifyAll 实现的可重入悲观锁。仅用于理解锁的基本原理，生产环境应使用 ReentrantLock。
 */
public class PessimisticLock {
    // 当前持有锁的线程，null 表示锁未被占用
    private Thread owner;
    // 当前线程的重入次数
    private int holdCount;

    /**
     * 获取锁。锁被其他线程持有时，当前线程进入等待；持有锁的线程可以重复获取，实现可重入。
     */
    ...
    /**
     * 释放锁。只有持有锁的线程才能释放；重入次数归零时才真正释放，并唤醒等待线程。
     */
...
### 死锁✅
死锁是一组线程形成循环等待，每个线程都持有其他线程需要的资源，同时等待其他线程释放资源，导致这组线程永久无法继续执行。
...
1. 多个线程加锁顺序不一致：线程 A 先获取锁 1、再等待锁 2；线程 B 先获取锁 2、再等待锁 1，这是最典型的死锁。
2. 同一线程重复获取不可重入锁：线程已经持有锁，又再次申请同一把不可重入锁，导致线程等待自己释放锁，形成自我死锁。
3. 读锁升级为写锁：线程持有 `ReentrantReadWriteLock` 的读锁时直接申请写锁，需要等待所有读锁释放，但它自己又没有释放读锁，可能导致死锁。
4. 持锁等待其他任务：线程持有锁后调用 `Future.get()`、`CountDownLatch.await()`等方法，而被等待的任务又需要获取这把锁才能完成，双方会相互等待。
5. 线程池饥饿死锁：线程池中的任务提交子任务到同一个已满的线程池，并同步等待子任务结果；工作线程都被父任务占用，子任务无法执行。
6. 多种资源获取顺序不一致：线程不仅可能竞争 Java 锁，也可能同时竞争数据库锁、连接池资源或分布式锁。如果不同线程获取这些资源的顺序不同，也可能形成循环等待。
...
jcmd <PID> Thread.print -l // 也可以使用 jcmd

jconsole // 唤醒图形化界面，然后选择线程->检测死锁
```

如果检测到典型的 Java 锁死锁，输出中通常会出现：`Found one Java-level deadlock: ...`

#### 如何预防和避免死锁
预防死锁的核心是破坏死锁产生的必要条件，实际开发中通常采用以下措施：

- **破坏循环等待条件**：靠按序申请资源来预防。按某一顺序申请资源，释放资源则反序释放。破坏循环等待条件。
  1. 统一加锁顺序：多个线程需要获取多把锁时，必须按照固定顺序获取，并按照相反顺序释放，避免形成循环等待。
  2. 减少锁的嵌套：尽量不要在持有一把锁时继续申请其他锁；如果必须嵌套，应明确所有锁的获取顺序。
- **破坏不剥夺条件**：占用部分资源的线程进一步申请其他资源时，如果申请不到，可以主动释放它占有的资源。
  3. 使用超时获取锁：使用 `ReentrantLock.tryLock(timeout, unit)`，获取超时后主动释放已经持有的锁并进行重试，避免无限等待。
- 其他
  4. 保证锁正常释放：将 `unlock()`放在 `finally`中，防止业务异常导致锁无法释放。
  5. 缩小锁的范围：只锁住真正需要互斥的代码，避免持锁执行网络请求、数据库操作、文件 IO 等耗时或不可控操作。
  6. 避免读锁直接升级：使用 `ReentrantReadWriteLock`时，应先释放读锁，再获取写锁，并在获得写锁后重新检查共享数据。
  7. 避免持锁等待其他任务：不要在持锁期间调用 `Future.get()`、`CountDownLatch.await()`等阻塞方法，特别是被等待的任务可能也需要当前锁时。
  8. 避免线程池饥饿：不要让线程池中的父任务同步等待提交到同一线程池的子任务，可以拆分线程池或改用异步编排。
  9. 优先使用高层并发工具：根据场景使用并发容器、原子类、`Semaphore`等成熟工具，减少手动管理多把锁。
- 破坏请求与保持条件：一次性申请所有的资源。

其中最常用的手段是统一加锁顺序、减少嵌套锁和使用超时锁。银行家算法也可以通过分配前进行安全性检查来避免死锁，但主要用于操作系统理论，普通 Java 业务开发中较少直接使用。


#### 银行家算法
在资源分配时，借助于银行家算法对资源分配进行计算评估，使其进入安全状态，可以避免死锁。

> 银行家算法是操作系统理论中的资源分配算法，普通 Java 业务开发几乎不会使用。
...
> `ThreadLocalMap`可以理解为一个定制化的 `HashMap`(`ThreadLocalMap`是专门实现的散列表，采用开放寻址法解决冲突，没有链表和红黑树。)，`key` 是 `ThreadLocal` 对象，`value` 是存储的值。
...
`ThreadLocalMap`使用弱引用的 key，是为了降低内存泄漏风险。如果直接使用 `HashMap`，Map 会同时强引用 `ThreadLocal`对象和 value。即使业务代码已经不再使用某个 `ThreadLocal`，只要线程仍然存活，这两者就无法被回收；在线程池等长生命周期线程中，泄漏反而会更严重。

使用弱引用后，外部不再强引用 `ThreadLocal`时，GC 可以回收 key，使其变为 `null`。`ThreadLocalMap`便能识别这个过期条目，并在后续调用 `get()`、`set()`或`remove()`时清理对应的 value。不过这种机制不能完全避免泄漏，因此使用完后仍应在 `finally`中调用 `remove()`。
...
### OOM和GC频繁
OOM 表示 JVM 无法继续为某类资源分配内存，或者操作系统资源限制导致分配失败，原因不一定是 Java 堆不足；GC 频繁则表示内存回收压力较大，也不一定最终会发生 OOM。排查时应先根据异常信息和 GC 日志确认问题属于哪块内存，再选择对应工具。

常见 OOM 类型：

- `Java heap space`：堆空间不足，可能是对象分配过快、堆容量不足或内存泄漏。
- `GC overhead limit exceeded`：大部分时间用于 GC，但回收效果很差。
- `Metaspace`：类元数据占满，常见于动态生成类过多或类加载器无法卸载。
- `Direct buffer memory`：直接内存不足，常见于 NIO、Netty 等堆外内存使用场景。
- `unable to create native thread`：线程数量过多、线程栈占用过大或系统资源受限。
- 容器 `OOMKilled`：进程被操作系统或容器强制终止，可能没有抛出 Java OOM，需要检查容器内存限制、进程 RSS 和系统日志。

### 线上OOM怎么排查
线上 OOM 应先保留现场并恢复服务，再根据 OOM 类型选择分析手段。日志和 Dump 解决的问题不同，不是简单二选一。

1. 保留现场：保存应用日志、GC 日志、监控数据、线程栈和已有的堆转储。实例持续影响流量时先摘流，取证后再重启恢复。
2. 判断类型：通过异常信息确认是堆、元空间、直接内存、线程创建失败，还是容器 `OOMKilled`。
3. 查看轻量级证据：结合 OOM 前后的流量、发布记录、堆和非堆内存、GC 次数与耗时、线程数及进程 RSS，判断是突发流量、容量不足还是持续泄漏。容器被杀时还要检查 cgroup 的 `memory.current`、`memory.max`和`memory.events`。
4. 获取对应现场：堆 OOM 重点分析 Heap Dump；元空间问题结合 `VM.classloader_stats`、NMT 和 Heap Dump 中的类加载器引用链；直接内存问题结合 NMT；线程创建失败重点检查线程转储、线程数量、`-Xss`、`ulimit`和容器 `pids.max`。
5. 定位根因：区分对象泄漏、分配速率过高、类加载器泄漏、直接内存未释放、线程过多等情况，再修改代码或调整容量，不能一上来只扩大内存。

生产环境应提前开启 OOM 自动转储，并确认目录可写且磁盘配额足够：
...
如果没有自动生成 Dump，但进程仍然存活，可以在确认磁盘空间充足、服务能够承受停顿后手动执行：

```bash
jcmd <PID> GC.heap_dump /data/dump/heap.hprof
```

手动 Heap Dump 可能引发较长时间的 STW 和大量磁盘 IO，线上不能直接执行。对于容器 `OOMKilled`、直接内存不足和线程创建失败，Heap Dump 也不一定能反映根因。

### 老年代飙高应该关注哪些指标
这个问题主要针对分代收集器。最重要的不是老年代瞬时使用率，而是每次 GC 后老年代的剩余占用，也就是回收后的内存基线；非分代 ZGC、Shenandoah 等收集器不能直接套用老年代指标。

1. GC 后老年代占用：如果回收后的最低点持续升高，说明长期存活对象不断增加，可能存在内存泄漏或常驻数据过多。
2. 老年代增长速度：回收后下降明显但很快再次涨满，通常表示对象分配速率过高或晋升过快。
3. 对象晋升速率：大量对象从年轻代进入老年代，可能是 Survivor 空间不足、对象存活时间较长或大对象较多。
4. Full GC、Mixed GC 或并发回收周期：关注触发频率、累计耗时、单次停顿和回收效果。
5. GC Cause：结合日志确认是否由分配失败、晋升失败、并发模式失败、`System.gc()`或元空间不足触发。
6. 大对象占用：使用 G1 时还应关注 Humongous Region，大对象可能直接占用多个 Region 并增加回收压力。

典型的泄漏趋势是老年代使用量呈锯齿状变化，但每次 GC 后的最低点越来越高，最终接近堆上限。

### 线上Full GC频繁怎么排查处理
这个问题主要针对会发生 Full GC 的分代收集器。排查时不能只看次数，应先结合 GC Cause 和回收后的内存水位判断根因，再决定是修改代码还是调整 JVM 参数；ZGC、Shenandoah 等收集器应按其自身回收周期和指标分析。

1. 确认现象：查看 Full GC 的频率、累计耗时、单次 STW 时长和每次回收前后的各内存区域占用。
2. 判断方向：Full GC 后老年代仍然很高，可能是内存泄漏或常驻数据过多；回收后下降明显但很快再次涨满，通常是分配过快或对象晋升过快；老年代不高仍频繁 Full GC，则要检查元空间、显式 `System.gc()`及收集器特有的失败原因。
3. 查看触发原因：结合 GC 日志确认是分配失败、晋升失败、并发回收失败、元空间不足还是显式 GC。
4. 定位对象：怀疑堆内存泄漏时获取 Heap Dump，按照后文“内存泄漏怎么定位”和“MAT 怎么分析堆转储文件”的步骤排查。
5. 对症处理：无界缓存和静态集合要增加容量与淘汰策略；大查询和大对象改为分页、分批或流式处理；晋升过快应结合对象年龄和 Survivor 使用情况处理；类加载器泄漏要修复动态类或类加载器生命周期。
6. 验证效果：修改后灰度观察相同负载下的 GC 频率、停顿时间、分配速率和 GC 后老年代基线，不要仅通过扩大堆内存掩盖问题。
...
CMS 的 `concurrent mode failure`、碎片和相关调优参数只适用于使用 CMS 的旧版本 JVM；G1 等收集器应结合自身的并发标记、Mixed GC、Humongous Region 和回收失败日志分析。

### 内存泄漏怎么定位
Java 内存泄漏是指对象在业务上已经不再需要，但仍然存在从 GC Roots 到该对象的强引用链，导致 GC 无法回收。定位时应先确认是否真的存在泄漏，再查找错误的引用关系。

1. 确认趋势：在相近负载下观察多轮 GC 后的堆占用。如果回收后的内存基线持续升高，才重点怀疑泄漏；如果每次都能明显回落，则更可能是分配速率过高。
2. 保留现场：提前开启 OOM 自动 Heap Dump；问题尚未触发 OOM 时，可以在评估停顿和磁盘风险后获取不同时刻的两份 Dump。
3. 找增长对象：对比两份 Dump 中各类对象的数量和 Retained Heap，找出持续增长且无法回收的对象。
4. 查引用链：通过 MAT 的 Dominator Tree 和 Path to GC Roots，定位哪个静态字段、线程、`ThreadLocal`、缓存、监听器或类加载器仍然持有对象。
5. 回到代码验证：确认对象是否应该长期存活，修复无界集合、缓存无淘汰、监听器未注销、`ThreadLocal`未清理或类加载器泄漏等问题。
6. 验证修复：在相同压测或流量下再次观察 GC 后内存基线，并对比新的 Heap Dump，确认对象数量不再持续增长。

如果 Java 堆占用稳定，但进程 RSS 持续增长，应转向排查直接内存、线程栈、JNI 和其他 Native Memory。可以在 JVM 启动时开启 NMT，并通过以下命令查看：
...
NMT 只能统计 JVM 能够跟踪的本地内存，并不能覆盖所有 JNI 或第三方本地库分配，必要时还要结合操作系统指标和 Native Profiler。未关闭的文件句柄、Socket 和数据库连接通常属于资源泄漏，不应只靠 Heap Dump 判断，还要结合连接池指标和操作系统资源监控。

### MAT怎么分析堆转储文件
MAT 分析 Heap Dump 的核心是先找出谁占用了大量内存，再通过 GC Roots 引用链确认对象为什么无法回收。

1. 打开 `.hprof`文件并等待 MAT 建立索引，先查看 Leak Suspects Report，但自动报告只能作为线索，不能直接当作结论。
2. 查看 Histogram，按类统计对象数量和 Shallow Heap，找出数量异常或增长明显的类。
3. 查看 Dominator Tree，按 Retained Heap 排序。Shallow Heap 是对象自身大小，Retained Heap 是该对象被回收后能够一并释放的总内存，后者更适合判断真实影响。
4. 对可疑对象执行 Path to GC Roots，排除弱引用、软引用和虚引用，沿强引用链查到静态字段、线程、`ThreadLocalMap`、集合或类加载器等真正持有者。
5. 怀疑类加载器泄漏时，使用 Class Loader Explorer 检查同一类是否被多个类加载器重复加载，以及旧类加载器为什么无法回收。
6. 有两份不同时刻的 Dump 时进行对比，重点查看对象数量和 Retained Heap 持续增长的类，避免把正常的大对象误判为泄漏。
7. 根据引用链回到代码定位创建和持有对象的位置，修复后重新压测并观察 GC 后的内存基线。
...
#### 线上使用什么垃圾回收器，为什么选择它
我们线上没有所有应用统一使用同一种垃圾回收器，而是根据 JDK 版本、堆大小、延迟目标和应用现状选择。Java 8 应用 使用的是 ParNew + CMS，并且约八成使用的是ParNew + CMS，对应的内存池为 Par Eden、Par Survivor 和 CMS Old Gen。部分升级到 JDK 11 的应用开始使用 G1。

选择 CMS 的主要原因是应用对接口延迟比较敏感。ParNew 负责回收新生代，CMS 负责回收老年代；CMS 的大部分工作可以与业务线程并发执行，相比 Parallel Old 能减少老年代回收造成的长时间 STW。对于堆规模在几 GB、已经稳定运行多年的 Java 8 在线服务，继续使用经过验证的 CMS，迁移风险通常也比直接更换收集器低。

CMS 也存在明显缺点：它采用标记清除算法，会产生内存碎片；并发回收期间业务线程还会继续分配对象，需要预留浮动垃圾空间；如果启动太晚，可能发生 `concurrent mode failure`，退化为 Serial Old 执行 Full GC。因此选择 CMS 后，重点是控制对象晋升速度，并保证 CMS 能在老年代耗尽前完成回收。

G1 更适合较新的 JDK 和较大的堆。它将堆划分为多个 Region，通过停顿预测模型选择回收集合，可以同时回收新生代和老年代。升级到 JDK 11 后通常优先评估 G1，但不能只因为 G1 更新就直接切换，仍要通过压测和灰度比较吞吐量、停顿时间、CPU 消耗和内存水位。
...
#### GC参数怎么调，调优依据是什么
GC 调优不是先背一组参数，而是先确定业务目标，再根据监控和 GC 日志定位主要矛盾。目标通常包括接口 P99/P999、吞吐量、可接受的最大停顿和容器内存上限。

1. 建立基线：在正常流量和高峰流量下记录 Young GC 频率与停顿、老年代回收周期、GC 总耗时占比、对象分配速率、晋升速率、GC 后老年代水位、CPU 和业务 P99。
2. 判断问题类型：Young GC 频繁但停顿很短，不一定需要处理；单次 Young GC 很慢，要检查新生代是否过大或存活对象是否过多；老年代增长过快，要检查对象晋升和长期存活对象；Full GC 频繁，则要先根据 GC Cause 判断是空间不足、晋升失败、并发回收失败、元空间不足还是显式 GC。
3. 针对性调整：堆太小会导致 GC 频繁，堆太大则可能增加单次扫描成本，并挤压直接内存、线程栈和元空间。容器环境中还必须给 Native Memory 留出余量，不能让 `-Xmx`接近容器内存上限。
4. 调整新生代：新生代太小会导致 Young GC 频繁，太大则可能增加单次复制量和停顿时间。Survivor 区太小会导致对象提前晋升，是否调整要结合年龄分布、Survivor 使用率和晋升量判断。
5. 调整老年代回收时机：CMS 启动太晚可能来不及回收，太早则会增加后台 CPU 消耗。需要根据老年代增长速度、一次 CMS 周期耗时和剩余空间反推安全阈值。
6. 验证效果：每次只调整一个主要变量，在相同流量或压测模型下比较修改前后的 P99、吞吐量、GC 停顿和回收后内存水位，再经过灰度观察决定是否全量。

Java 8 应用常见的 CMS 参数基线如下，但具体数值必须根据应用数据验证，不能直接照抄：
...
`CMSInitiatingOccupancyFraction=80`表示老年代占用达到约 80%时启动 CMS；配合 `UseCMSInitiatingOccupancyOnly`后固定使用该阈值，不再让 JVM 自适应调整。阈值过高可能导致 CMS 来不及完成，阈值过低则会增加回收次数和 CPU 消耗。

`SurvivorRatio=10`表示 Eden 与单个 Survivor 区的容量比例约为 10:1。它不能单独决定对象何时晋升，还需要结合动态年龄判定和 `MaxTenuringThreshold`。如果 Survivor 经常放不下存活对象，应先判断是 Survivor 太小，还是业务确实产生了大量中等生命周期对象。

如果使用 G1，主要关注 `MaxGCPauseMillis`、`InitiatingHeapOccupancyPercent`和 Region 大小。`MaxGCPauseMillis`只是软目标，不是停顿上限；设置得过低可能让 GC 更频繁并消耗更多 CPU。JDK 11 的 G1 默认使用自适应 IHOP，此时 `InitiatingHeapOccupancyPercent`主要提供初始参考，JVM 会根据标记耗时和分配速度动态调整；只有关闭自适应 IHOP 后，才更接近固定触发阈值。Region 大小通常由 JVM 根据堆大小自动选择，它还会影响 Humongous Object 的判定，不应在缺少数据时手工指定。CMS 的新生代和老年代参数也不能直接照搬到 G1。

#### 生产环境的GC频率多少算正常
GC 没有统一的正常频率，不能只根据“几分钟一次”判断。应该同时观察单次停顿、GC 累计耗时占比、业务延迟以及 GC 后内存是否正常回落。

以一个约 4GB 堆的在线服务为例，稳定基线是每个实例每天约 370～380次 Young GC，约 3～4分钟一次，单次平均停顿约 15ms；堆利用率均值约 35%～42%，峰值约 50%～60%。在接口 P99 稳定、GC 总耗时占比很低、老年代基线没有持续上涨的情况下，这种频率是可以接受的。

如果 Young GC 从几分钟一次突然变成几秒一次，需要检查流量、对象分配速率、大批量查询和新生代大小。即使频率没有明显变化，如果单次停顿从几十毫秒升到几百毫秒，并开始影响接口 P99，也属于异常。因此判断 GC 是否正常，应将当前稳定基线与相同流量下的历史数据比较，而不是套用固定次数。

#### Full GC多久一次算异常
对于低延迟在线应用，Full GC 的理想状态是尽量不发生，但不能只按固定时间判断。一次 Full GC 是否异常，取决于触发原因、停顿时间、回收效果和业务影响。

首先要区分 CMS 周期和 Full GC。CMS 正常完成一次并发老年代回收，不等于发生一次 Full GC；CMS 的 Initial Mark 和 Remark 阶段会 STW，但大部分阶段与业务线程并发。并发回收失败、晋升失败或元空间不足等情况，可能触发或退化为真正的 Full GC。显式调用 `System.gc()`也可能触发 Full GC，但如果 CMS 同时启用了 `ExplicitGCInvokesConcurrent`，通常会请求一次并发 CMS 周期，而不一定执行传统的 STW Full GC，应以日志中的实际 GC 类型为准。

对于平时几乎没有 Full GC 的在线服务，滚动 24 小时出现一次就值得排查。如果数小时内反复发生、单次停顿达到秒级、回收后老年代仍然很高，或者已经影响接口 P99 和超时率，就可以明确判断为异常。发生在启动、发布或显式 GC 附近的 Full GC，则要结合参数和日志上下文判断，不能仅凭次数认定为内存泄漏。

常见原因和处理方向如下：老年代空间不足要检查晋升速度和长期存活对象；`concurrent mode failure`要检查 CMS 是否启动过晚；`promotion failed`要检查老年代连续空间和对象晋升量；元空间触发要排查动态类生成和类加载器泄漏；显式 GC 要追踪调用方，并结合 `DisableExplicitGC`或`ExplicitGCInvokesConcurrent`判断实际行为。

#### GC日志里哪些指标要重点关注
分析 GC 日志时，可以按照“触发原因、停顿时间、回收效果、对象流向、失败信号”的顺序查看。

1. GC Cause：确认是 Eden 分配失败、CMS 触发、元空间不足、显式 `System.gc()`还是晋升失败。触发原因不同，处理方向完全不同。
2. 停顿时间：关注单次 Pause、平均值、最大值和高分位，而不能只看平均值。CMS 重点关注 Initial Mark、Remark 和 Young GC 的 STW 时间；并发阶段耗时较长不一定等于业务线程被暂停同样长时间。
3. 回收前后内存：观察 Eden、Survivor、老年代和元空间在 GC 前后的占用，判断回收了多少内存。GC 后老年代最低点持续升高，是内存泄漏或长期存活对象增加的重要信号。
4. 分配与晋升：观察对象分配速率、Survivor 使用率、对象年龄分布和每次 Young GC 晋升到老年代的大小。晋升过快会明显缩短老年代回收周期。
5. GC累计开销：观察一段时间内 GC 总耗时占比，并与 CPU、吞吐量和接口 P99 对齐。平均停顿正常但偶发长尾明显时，应继续检查最大值和具体时间点。
6. 失败与退化信号：重点搜索 `concurrent mode failure`、`promotion failed`、`Full GC`、`Metadata GC Threshold`和`System.gc()`。这些信号通常比单纯统计 GC 次数更能说明问题。

总结来说，垃圾回收器要根据 JDK 版本、堆大小和延迟目标选择；参数要根据分配速率、晋升速率和回收速度调整；GC 是否异常不能只看次数，而要看停顿、回收效果、内存趋势和业务 SLO。
